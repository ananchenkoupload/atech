...
selectCallback = function(variant, selector) {
  ...
  $('body').trigger('setQuickShopVariant', [{variant}])
  ...
  if (variant && variant.available == true) {
    if(variant.price < variant.compare_at_price){
      ...
      $('.savings', $product).html("You Save" + ' ' + parseInt(((variant.compare_at_price - variant.price) * 100.0 ) / variant.compare_at_price) + '% (' + '<span class="money">' + Shopify.formatMoney(variant.compare_at_price - variant.price, $productForm.data('money-format')) + '</span>)');
...
$(function() {
  ...
  var quickShop = {
    ...
    updateVariant: function(variant){
      ...
      if (globalQuickShopProduct != 'undefined'){
        ...
        //Title and Vendor
        var text = product.product_in_collection_url;
        var shortText = jQuery.trim(text).split("/").slice(3).join("/");
        $('.js-product-title').html('<a href="https://aromatechscent.com/'+ shortText +'" title="'+ product.escaped_title +'">'+ product.title +'</a>');
        $('.js-product-vendor')
        ...
        var productDetails = '<a href="https://aromatechscent.com/'+ shortText +'" class="secondary_button" title="'+ product.escaped_title +' Details">'+ product.details_text +'</a>';
  ...
  function ajaxUpdateCart(lineID, quantity, parent, properties) {
    $.ajax({
      url: '/cart/change.js',
      dataType: 'json',
      cache: false,
      type: 'post',
      data: { quantity: quantity, line: lineID , properties: {
  '_Want to have different scents in your pack? Please advise in the below text field.': properties
  }},
      success: function (data) {
        refreshCart(data);
  ...
  // Extend - Used to refresh the cart when the minicart is being normalized             
  $('body').on('refreshCart', function(e, cart){
     refreshCart(cart.cart)
  })
  
  // Extend - Set variantId on product-page
  $('.product_section form').on('change', function(e) {
      var extendOfferDiv = $('.product_section').find('#extend-offer').get(0) 
  	  var urlParams = new URLSearchParams(window.location.search);
      var variantId = urlParams.get('variant')
      if(variantId){
        Extend.setActiveProduct(extendOfferDiv, variantId)
      }
  ...
  function refreshCart(cart) {
    ...
    if (cart.item_count == 0) {
      ...
      $.each(cart.items, function(index, item) {
        ...
        // Render Extend title for warrant products on 2nd minicart
        if(item.vendor === 'Extend'){
        	cart_items_html += '<div class="cart_item__title"><div class="item_title"> Extend Protection Plan' 
        }else{
        ...
        if(item.properties) {
          $.each(item.properties, function(title, value) {
            // dont render Extend product properties if on 2nd mini cart
            if (value && item.vendor !== 'Extend') {
        ...
        cart_items_html += '<div class="left product-quantity-box">' +
          ...
          '</div>' + '<div id="extend-cart-offer" data-extend-variant="' + item.variant_id + '" data-extend-quantity="' + item.quantity + '"></div>' +
        '</div>';
    ...
    /* 
    	EXTEND - call custom event to re-add Extend warranties since it re-renders the entire cart upon any action.
    */
    document.dispatchEvent(new CustomEvent('buildCart'))
          ...
          success: function(data) {
            ...
            if(data.meta.code === 200 && data.data.length) {
              ...
              for(var i = 0; i < data.length; i++) {
                ...
                if(item) {
                  item = '<div class="'+s.column+ ' columns instagram__item small-down--one-half">'+item+'</div>';
                }
                if(item !== '') {
                  s.el.append(item);
                  count++;
